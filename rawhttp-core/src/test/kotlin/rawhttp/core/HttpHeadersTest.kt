...
class HttpHeadersTest {
    ...
    fun headerNamesCanBeRemoved() {
        ...
        basicHeaders.except().run {
            headerNames shouldBe listOf("Hello", "Foo", "BaR")
            size() shouldBe 3
        }
        basicHeaders.except("Nothing").run {
            headerNames shouldBe listOf("Hello", "Foo", "BaR")
            size() shouldBe 3
        }
        basicHeaders.except("Hello", "BAR").run {
            headerNames shouldBe listOf("Foo")
            get("FOO") shouldBe listOf("bar")
            size() shouldBe 1
        }
        basicHeaders.except("FOO", "BAR", "NONE", "ZORT", "Hello").run {
            headerNames shouldHaveSize 0
            size() shouldBe 0
        }
    }
